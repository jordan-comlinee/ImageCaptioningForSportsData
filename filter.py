...
class coco_supercategory_filter:
    ...
    def __init__(self, json_path, caption_json_path, imgs_dir, scateg='sports'):
        self.coco = COCO(json_path)  # instanciate coco class
        self.cocoCaption = COCO(caption_json_path)
    ...
    def get_imgs_from_json(self):
        ...
        print(f"{len(images)} images in '{self.caption_json_path}' with '{self.scateg}' captions")
    ...
    def filter_caption_json_by_category(self, new_json_path):
        """creates a new json with the desired category"""
        # {'supercategory': 'person', 'id': 1, 'name': 'person'}
        ### Filter images:
        print("Filtering the annotations ... ")
        ...
        os.makedirs(json_parent, exist_ok=True)
        imgs_ids = [x['id'] for x in self.images]  # get img_ids of imgs with the category
        new_imgs = [x for x in self.cocoCaption.dataset['images'] if x['id'] in imgs_ids]
        catIds = self.catIds
        ### Filter annotations
        new_annots = [x for x in self.cocoCaption.dataset['annotations'] if x['image_id'] in imgs_ids]
        ### Reorganize the ids
        new_imgs, annotations = self.modify_ids(new_imgs, new_annots)
        data = {
            "info": self.cocoCaption.dataset['info'],
            "licenses": self.cocoCaption.dataset['licenses'],
            "images": new_imgs,
            "annotations": new_annots
        }
        print("saving json: ")
...
def main(subset, year, root_dir, category='sports'):
    ...
    caption_json_file = join(os.path.split(root_dir)[0], 'captions_' + subset + year + '.json')  # local path
    imgs_dir = join(root_dir, category + '_' + subset)
    new_json_file = join(root_dir, 'annotations', 'captions_'+ subset + year + ".json")
    coco_filter = coco_supercategory_filter(json_file, caption_json_file, imgs_dir, scateg=category)  # instanciate class
    # coco_filter.save_imgs()
    coco_filter.filter_caption_json_by_category(new_json_file)
